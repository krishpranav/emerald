...
func (a *APU) squareSample(ch int) int8 {
	...
	if ch == 0 {
		sweepTime := (a.Load32(SOUND1CNT_L) >> 4) & 0b111
...
func (a *APU) waveSample() int8 {
	...
	cnth := uint16(a.Load32(SOUND3CNT_H))
...
func (a *APU) SoundClock(cycles uint32) {
	...
	cnth := uint16(a.Load32(SOUNDCNT_H))
	...
	for sndCycles >= SAMP_CYCLES {
		...
		cntl := uint16(a.Load32(SOUNDCNT_L))
		for i := 0; i < 4; i++ {
...
func (a *APU) _resetSoundChan(ch int, enable bool) {
	if enable {
		...
		case 2:
			a.waveReset()
		case 3:
			if util.Bit(a.Load32(SOUND4CNT_H), 3) {
				a.chans[3].lfsr = 0x0040
			} else {
				a.chans[3].lfsr = 0x4000
			}
		}
